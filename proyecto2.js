...
function searchF() {
  ...
  function sugerencias(){
      async function sugerenciasbusc(resultado) {
        let url = `https://api.giphy.com/v1/gifs/search/tags?api_key=og2cGOovwMy2VkjKm8PCJRTApTXFM8BJ&q=${resultado}&limit=5&offset=0&rating=g&lang=en`;
        const resp = await fetch(url);
        const data = await resp.json();
        return data;
      }
        let info = sugerenciasarbusc(buscar.value);
        info.then(response=> {
            console.log(response);
        for(let i=0 ; i<6 ; i++) {
          let sugeren= document.createElement("div");
          sugeren.setAttribute("class", busc);
          sugeren.innerHTML= `${buscar.value}`;
          busc.appendChild(sugeren);
...
function flechaDerecha() {
    ...
    let datos = buscarFlecha();
    gifTrending.innerHTML="";
    datos.then(response=> {
    for(let i = count_2; count_2<=i<=(count_2+2) ;i++){
     let gifo = document.createElement('img');
     gifo.setAttribute("src",response.data[i].images.original.url);
     gifo.setAttribute("class", "gifs");
     gifTrending.appendChild(gifo); }
...
botonDerH.addEventListener("click", flechaDerecha());
...
buscar.addEventListener("input", 
  ...
  function searchMas() {
    async function buscarGifos(valor) {
        ...
        let url = `https://api.giphy.com/v1/gifs/search?api_key=${ApiKey}&q=${valor}&limit=${limit}&offset=0&rating=g&lang=en`;
        const resp = await fetch(url);
        const info = await resp.json();
        return info;
    }
